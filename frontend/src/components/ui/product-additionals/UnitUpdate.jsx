...
} from "../../../redux/product-additionals-state/unitApi";

const UnitUpdate = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const { data, isLoading: isDataLoading, error } = useSingleUnitQuery(id);
  ...
  useEffect(() => {
    if (data?.anUnit) {
      setElement({
        UnitName: data.anUnit.UnitName || "",
        description: data.anUnit.description || "",
      });
    }
  }, [data?.anUnit]);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(element);
    try {
      await updateUnit({ id: id, status: element }).unwrap();
