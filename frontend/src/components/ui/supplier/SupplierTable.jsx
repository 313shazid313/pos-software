...
const SupplierTable = () => {
  let serial = 0;
  const { data, isError, isLoading, refetch } = useGetAllSupplierQuery();

  console.log(data);

  const [deleteaSupplier] = useDeleteaSupplierMutation();

  const handleSupplierDelete = async (id) => {
    try {
      if (confirm("Sure You Want to Delete")) {
        const response = await deleteaSupplier(id).unwrap();
